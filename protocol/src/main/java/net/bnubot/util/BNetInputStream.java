 ...
 * $Id: BNetInputStream.java 1604 2008-09-11 17:05:34Z scotta $
 */
...
public class BNetInputStream extends DataInputStream {
	...
	public short readWord() throws IOException {
		return (short) (((readByte() << 0) & 0x00FF) | ((readByte() << 8) & 0xFF00));
	}
	...
	/**
	 * Bytes for this operation are read from the parameter
	 * <code>data[off..off+3]</code>.
	 * 
	 * @param data
	 *            the byte array to read the double-word from.
	 * @param off
	 *            the index to start reading from <code>data</code>.
	 * @return the next double-word of this input stream as a signed 32-bit
	 *         <code>int</code>.
	 * @exception IOException
	 *                the stream has been closed and the contained input stream
		...
		do {
			...
			if (c == 0)
				throw new IOException(
						"Read a null, looking for comma terminator");
			if (c == ',')
				return out;
			out += (char) c;
		} while (true);
