 ...
 * $Id: URLDownloader.java 1810 2009-06-22 17:33:07Z scotta $
 */
...
public class URLDownloader {
			...
			if (sha1 == null) {
				...
				if (fSHA1.equals(sha1)) {
					if (Out.isDebug(URLDownloader.class))
						Out.debugAlways(URLDownloader.class, "SHA1 match for "
								+ to.getName());
					return;
				}
				Out.error(URLDownloader.class,
						"SHA1 mismatch for " + to.getName() + "\nExpected: "
								+ sha1 + "\nCalculated: " + fSHA1);
			}
		}
	...
	public static void downloadURLNow(URL url, File to, SHA1Sum sha1,
			...
			for (int i = 0; i < folders.length(); i++) {
				...
				if (path.endsWith(sep)) {
					...
					if (!f.exists())
						f.mkdir();
					if (!f.isDirectory()) {
						Out.error(URLDownloader.class, path
								+ " is not a directory!");
						return;
					}
				}
