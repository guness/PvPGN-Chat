 ...
 * $Id: GenericCrypto.java 1590 2008-08-27 18:24:10Z scotta $
 */
...
public class GenericCrypto {
	...
	public static ByteArray decode(ByteArray text) {
		...
		case (byte) 0xB7:
			return new ByteArray("{Reverse} ").concat(decode(ReverseCrypto
					.decode(text.removeFirst())));
		case (byte) 0xB8:
			return new ByteArray("{MC} ").concat(decode(MCCrypto.decode(text
					.removeFirst())));
		case (byte) 0xA4:
			return new ByteArray("{DM} ").concat(decode(DMCrypto.decode(text
					.removeFirst())));
		case (byte) 0xA3:
			try {
				return new ByteArray("{HEX} ").concat(decode(HexDump
						.decode(text.removeFirst())));
			} catch (Exception e) {
				return new ByteArray("{INVALID HEX} ").concat(text);
			}
		case (byte) 0xE6:
			return new ByteArray("{B64} ").concat(decode(Base64.decode(text
					.removeFirst())));
		}
		return text;
	}
	...
	public static ByteArray encode(ByteArray data, int crypto) {
		if ((crypto & CRYPTO_REVERSE) != 0)
			data = new ByteArray((byte) 0xB7)
					.concat(ReverseCrypto.encode(data));
