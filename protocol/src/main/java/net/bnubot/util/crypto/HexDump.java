 ...
 * $Id: HexDump.java 1590 2008-08-27 18:24:10Z scotta $
 */
...
public class HexDump {
	...
	public static String DWordToIP(int dword) {
		...
		return Integer.toString(bytes[0]) + "." + Integer.toString(bytes[1])
	...
	public static int PrettyToDWord(String pretty) {
		...
		return ((bytes[3] << 0) & 0x000000FF) | ((bytes[2] << 8) & 0x0000FF00)
	...
	public static int StringToDWord(String str) {
		...
		if (bytes.length != 4)
			throw new InvalidParameterException("bytes.length != 4");
		return ((bytes[0] << 0) & 0x000000FF) | ((bytes[1] << 8) & 0x0000FF00)
	...
	public static String toHexWord(int b) {
		return hexChr((b & 0xF000) >> 12) + hexChr((b & 0x0F00) >> 8)
				+ hexChr((b & 0x00F0) >> 4) + hexChr(b & 0x000F);
	}
	...
	public static String hexDump(byte data[]) {
		...
		for (int offset = 0; offset < data.length; offset += 16) {
			...
			for (int i = 0; i < end; i++) {
				...
				output += Character.valueOf((char) b);
			}
		}
		return output;
	}
