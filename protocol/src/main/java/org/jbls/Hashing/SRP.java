/**
 * $Id: SRP.java 1608 2008-09-12 17:35:39Z scotta $
 */
...
public class SRP {
	...
	public SRP(String username, String password) {
		this.username = username.toUpperCase(Locale.getDefault());
		this.password = password.toUpperCase(Locale.getDefault());
	...
	public byte[] get_S(byte[] s, byte[] B) {
		// S = (B - v)^(a + ux) % N
		BigIntegerEx S_base = N
				.add(new BigIntegerEx(BigIntegerEx.LITTLE_ENDIAN, B))
				.subtract(get_v(s)).mod(N);
